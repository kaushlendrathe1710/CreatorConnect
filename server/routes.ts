...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ===== ADMIN ROUTES =====
  const { isAdmin } = await import("./adminMiddleware");
  ...
  app.get("/api/admin/stats", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      const allUsers = await storage.searchUsers('');
      const allPosts = await storage.searchPostsByHashtag('');
      const totalUsers = allUsers.length;
      const totalCreators = allUsers.filter((u: any) => u.isCreator).length;
      const totalAdmins = allUsers.filter((u: any) => u.isAdmin).length;
  ...
  app.patch("/api/admin/users/:userId/admin", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
      ...
      if (isSuperAdmin(userToUpdate.email!)) {
